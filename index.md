...
- Auth bootstrap attempts a cookie refresh when `/auth/me` is stale, so remembered-device sessions can survive page reloads after the short JWT expiry as long as the refresh window is still valid.
