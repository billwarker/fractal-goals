...
Revision `b2c4d6e8f0a1` promotes tags into a fractal-owned catalog. `ActivityTagDefinition` owns canonical display name, color, picker order, optimistic version, archive state, and selected/global applicability; `ActivityTag` is now the stable per-activity binding retained by instance/set assignments and saved-view JSON so existing result history keeps its IDs. Global definitions materialize bindings for all current activities and `ActivityService` adds them to future activities. Circuit scopes reference catalog definitions directly instead of rediscovering ownership by name. The catalog service is the only CRUD boundary: it enforces normalized overlapping-name conflicts, previews destructive changes and merges, archives without changing history, transactionally merges duplicate definitions, and permanently deletes only after version plus name confirmation while removing assignments/view filters/circuit scopes and incrementing affected versions. The former activity-nested tag CRUD routes and client hooks have been removed; instance/set assignment routes remain activity-result operations. The migration keeps arbitrary legacy duplicates separate for review and consolidates only active tags demonstrably sharing a circuit scope.

Revision `c3d5e7f9a1b2` adds canonical metric precision. Existing integer and duration definitions retain their zero-place behavior, while decimal definitions migrate to a 2-place default; a database constraint and request validation keep configurable decimal and duration precision between 0 and 6.
...
- Goal/activity and goal/activity-group replacement writes reconcile membership instead of deleting and recreating unchanged junction rows. Exact add/remove deltas append transaction-bound, name-snapshotted `EventLog` records, and Goal Timeline reads those immutable events rather than treating current junction rows as historical evidence; revision `d4e6f8a1b3c5` backfills active legacy associations and adds the root/goal/time history index. The goal editor skips the batch request when its direct activity and group ID sets are unchanged, while real association mutations invalidate root-scoped Timeline queries immediately.
...
- Mobile session activity metric editors keep unit/history metadata visible beneath the metric-label column rather than beneath value fields for set-based, direct, and split variants. Desktop continues to show units and previous-value context inline. Metric keystrokes across direct instances, ordinary sets, split metrics, and circuit members use native uncontrolled inputs and produce no React render commits: ref-backed activity and circuit registries preserve drafts for cascade/add/remove operations, while normalization plus optimistic persistence remain blur-driven. External saved-value changes synchronize into idle inputs without overwriting a focused draft. This prevents circuit/set progress calculations, sibling editors, and the focused editor itself from rerendering on every character. Circuit member metric saves bypass the structural run mutation's global pending state, update the authoritative run response directly, and refresh activity/progress consumers in the background without redundantly refetching the run collection. Activity and circuit instance option menus portal outside Session Detail's clipped content tree and use the shared visual-viewport-aware anchored positioning contract, including collision clamping, vertical fallback, and focus restoration on Escape.
...
- The canonical Modal traps Tab/Shift+Tab focus, focuses its first control on open, restores prior focus only when the modal actually closes, supports Escape/backdrop policies, and respects the shared reduced-motion-safe animation. Close callbacks are read through synchronized refs so controlled form rerenders cannot restart focus management or steal focus from confirmation inputs. `ModalBackdrop` owns the shared visual-viewport contract for canonical and specialized dialogs: it follows `visualViewport` size and offsets across resize/scroll changes with dynamic-viewport fallback, preventing iOS browser chrome and keyboards from placing dialog controls outside the visible screen. Reserved-layout sheets that intentionally terminate above app-owned navigation can explicitly opt out. The canonical Modal additionally constrains its card through that contract, keeps its header/close affordance fixed, and makes only the body scroll with safe-area-aware outer gutters.
...
- round tags materialize directly onto each set result for set-based slots and each activity instance for setless slots. They do not duplicate circuit-run tags on sets because parent-instance tags already inherit hierarchically. The service creates one selected-scope catalog definition with bindings for participating activities when missing, consolidates equivalent participant definitions onto that identity, rejects archived-name reuse, and preserves assignments that predated a removed scope. Catalog renames and color changes propagate to directly referenced circuit scopes; circuit-dependent tags remain archive-protected until their scopes are removed, and descendant assignment endpoints reject attempts to remove scope-owned tags
...
- Ordinary activity timing uses `session_work_intervals` as the canonical accrual ledger while `ActivityInstance.time_start` / `time_stop` represent the logical timer lifecycle. A live ordinary activity (active or paused) permits a start-only correction: the service locks its session, moves the first accrual boundary, recomputes any closed-segment duration, and rejects chronology or cross-item overlaps while retaining the null logical stop. Historical one-sided timing creation remains invalid. Pausing a session closes the current accrual segment but retains a null logical stop and marks the activity paused; resuming opens a new segment, clears legacy stale stop markers, and makes the same activity visibly active again. Session-activity reads also project an open interval as `time_stop: null`, repairing presentation for already-affected rows without rewriting history. Removing a running activity closes its open interval before soft deletion. Genuine timer conflicts name the active activity when available and expose one inline `Complete it and switch` action in a non-layout-shifting overlay aligned to the activity card's right-side action stack. The notice has a dedicated dismiss control and automatically fades after ten seconds before clearing. Its switch action atomically closes and completes the displaced ordinary activity, starts the requested timer under the session lock, runs ordinary completion bookkeeping, and returns both activity records so the client replaces both cache entries without presenting concurrent timers. Expected conflicts remain inline and do not also raise a redundant error toast.
- Activity-instance autosaves preserve their serialized, newest-value-wins queue while returning completion results tied to the payload that is actually persisted. Timer start/stop drafts and relative adjustments wait for that result before clearing or closing; backend validation and interval-conflict failures remain visible inline with the unsaved draft instead of appearing to save and then snapping back after cache reconciliation.
...
The shared activity builder keeps its create/save actions in a persistent footer while the form body scrolls, and reflects the in-progress activity name in its header. Activity groups are selected before direct goal associations: choosing a group merges its linked goals into the activity draft, and the same selector can open the canonical group builder, automatically select the newly created group, and retain the unfinished activity. Activity creation launched from a Goal Details Activities picker seeds that goal as a visible direct association, preserves any additional goal selections, and persists the association in the activity-create request; the post-create hook only retains a compatibility repair for callers or older responses that do not confirm it. The shared form awaits entry-point post-create work before one canonical close transition, so the Activities view cannot return before its association cache is synchronized. Inline fractal-metric creation uses the canonical additive, multiplicative, trend-direction, and integer/decimal/duration input-type semantics instead of a name/unit-only shortcut; its compact, cancellable creator stays beneath the metric row that opened it and fills that exact slot on success. These behaviors are shared by Manage Activities and the Goal Details activity-creation flow. In the Manage Metrics library, selecting a metric card loads its values into the right-hand editor; cards keep only the distinct Copy and Delete actions, and changes remain explicit through Save Changes. Metric definitions carry a validated 0–6 decimal-place precision: decimal metrics default to 2, duration metrics preserve their zero-place default but can opt into fractional seconds, integer metrics are fixed at 0, and duration entry/formatting supports `MM:SS.ss` form when configured.
...
- session metric inputs honor metric input type, 0–6 place precision, default value, predefined allowed values, and min/max bounds
...
- fractal-owned catalog tags with stable per-activity bindings assigned directly to instances or sets; selected tags can span one or more activities, global tags bind to every current and future activity, and instance tags are inherited by every set at calculation time without being copied
- Manage Activity Tags is the complete responsive catalog surface: search covers names and applicable activities; filters cover lifecycle, scope, activity, instance usage, and duplicate status; rows use the same typography hierarchy as Manage Metrics and show only tagged-instance usage plus applicability; the filter bar wraps within its pane and filtered empty states use a dedicated readable layout. Editing includes accessible picker-order controls and selected/global applicability with scope-reduction impact confirmation; duplicate merge and permanent delete use plain-language server previews. Session Detail groups assigned, global, and activity-specific choices, reuses or extends matching definitions during creation, and exposes catalog edit/archive/permanent-delete correction workflows in the same focus-contained portalled picker
- archive preserves historical assignments and saved-view meaning while preventing new assignment. Hard delete intentionally removes every instance/set assignment, saved-view reference (including archived views), and circuit/round scope; it increments affected assignment/view versions and requires typed name confirmation when linked records exist. Merge keeps the chosen target name/color by default, unions applicability, rewrites linked tag IDs without duplicate junction rows, and can promote the result to global
