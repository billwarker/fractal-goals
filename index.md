...
- A user can have at most one unfinished session across all fractals. `sessions.owner_id` and the partial unique index `uq_sessions_one_active_per_owner` provide the database invariant, while session lifecycle preflight checks return a navigable `409 active_session_exists` response. The introducing migration preserves the most recently updated unfinished session per owner and marks older legacy duplicates complete without deleting their history. `GET /api/sessions/active` is the canonical user-wide lookup. Desktop and mobile navigation consume the shared active-session query: the action shows `+ ADD SESSION` only when the slot is empty, otherwise reuses `CompletionCheckBadge` to show `SESSION IN PROGRESS` or `SESSION PAUSED` and links directly to the session in its owning fractal. Creation, completion, reopening, deletion, pause, and resume mutations invalidate this shared state; stale direct visits to Create Session redirect to the existing active session. Completed quick-entry sessions never occupy the active slot.
