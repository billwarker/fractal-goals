...
- The public entry has a route-specific bootstrap (`PublicLandingRoot.jsx`) that excludes the authenticated provider/router tree; app routes load `AuthenticatedRoot.jsx` instead. `client/index.html` paints an accessible, route-gated critical hero shell before React starts, while monitoring initializes during idle time. The goals `FlowTree`, Features section, and individual feature previews load only as their panels approach or visitors signal intent, with stable section anchors and skeletons preserving snap navigation and layout. The Vite production build fails if the public landing's static initial import closure exceeds 175 kB gzip of JavaScript or 20 kB gzip of CSS; the July 2026 optimized build measures 135.4 kB JS and 9.5 kB CSS, down from the former 495.8 kB entry bundle.
