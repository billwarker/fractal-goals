...
- First-run guidance is gated by the admin-managed `onboarding_v1` flag and scoped per fractal. Fractal creation initializes an active, revisioned entry in `preferences.onboarding_by_root` within the same transaction; `GET/PATCH /api/auth/onboarding?root_id=...` isolates checklist presentation state, dismissal, visits, celebration, and server-derived completion facts to that owned root. Roots predating per-fractal state are silently grandfathered as dismissed. `OnboardingProvider` keys its query by user and root, renders the zero-fractal welcome, a compact lower-left checklist showing one step at a time with previous/next controls and automatic advancement after completion, and expands in place into a dense, internally scrolling sub-step guide. Settings provides resume/restart controls, while first-session completion retains its evidence celebration/tree highlight. Contextual “Got it” hint cards have been removed so the checklist is the sole onboarding guidance system.
- Activity create/update/delete paths invalidate the shared onboarding query prefix after persistence, so the server-derived “Create an activity and metric” fact refetches immediately and advances the checklist without waiting for stale time or navigation.
- Onboarding API version 2 adds per-parent `substeps` facts while retaining the seven canonical completion thresholds. The compact lower-left widget separates per-fractal local collapse from persisted dismissal; its single-line success-coloured progress action sits at the top-right and, like the parent `Go` action, expands the same fixed-width shell while preserving its typography hierarchy. The expanded view uses a bare parent step number aligned with the lettered sub-step codes, leaves `N/7` position context to the footer navigation, and maintains a stable right-side status column for completion checks. Actionable sub-step rows are full-width keyboard-accessible deep links to the relevant feature; informational guidance remains static, and completed state relies on the shared check badge without redundant row gradients.
- Root detection is limited to known fractal route sections so account/admin routes never issue root-scoped onboarding requests; fractal deletion removes its `onboarding_by_root` entry in the same transaction to prevent dead preference state.
...
- Mobile fractal routes use a fixed, safe-area-aware five-item bottom navigation for Goals, Programs, Sessions, Notes, and Analytics; the compact top bar retains fractal switching, session creation, and Settings.
- Session template name badges are shrink-safe and wrap-safe in both compact and expanded Sessions cards: the shared badge owns border-box sizing, vertical text breathing room, and a min-width-zero label, while the expanded title link gives the badge a real wrapping width instead of clipping long template names.
