...
- Create Session is program-aware on the user’s timezone date. `GET /api/<root_id>/programs/active-days?date=YYYY-MM-DD` is date-keyed in TanStack Query and returns stable program/day identity, program and template colors, template requirement order, program/block goal ids, completion counts, and completed template ids. The page groups by program id (so duplicate names never merge), advertises today’s day, prioritizes its templates, consistently styles program, block, and template names with their configured colors, auto-selects a single required session, and accepts calendar deep links via `program_day_id`. Selecting a program day enables a per-user/root/program lineage scope preference for the goal hierarchy; manual goals outside that advisory scope remain selected until explicitly removed. Session goal-scope previews expose authoritative `program_scope_goal_ids`, while creation remains permissive and records `off_program_goal_ids` in `session_data.program_context` for analytics. Calendar Day offers “Start session” only for today’s occurrences.
