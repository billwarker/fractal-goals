...
- Landing example publication is production-timeout hardened: snapshot enrichment reuses the already validated root, preloaded goal graph, effective levels, group activities, and direct activity IDs across per-goal services; phase logs report per-example build time plus commit, delivery, total duration, payload bytes, and delivery outcomes. Static GCS upload and edge-cache warming remain best-effort after the database commit but have strict configurable time budgets (`LANDING_EXAMPLES_STATIC_UPLOAD_TIMEOUT_SECONDS`, default 10s with retries disabled; `LANDING_CACHE_WARM_TIMEOUT_SECONDS`, default 2s), so delivery failure cannot consume the full request window. The dedicated Nginx publish location has an explicit 200s read budget and converts gateway failures to JSON, Gunicorn allows 210s, Cloud Run allows 240s, and the admin client allows 205s; normal production-shaped 17-goal publication is guarded below 1.8s and 65 database queries in the performance suite. Because commit precedes delivery, timeout copy tells admins to refresh before retrying, while repeated publication remains idempotent.
