...
- Onboarding API version 2 adds per-parent `substeps` facts. The checklist has six canonical steps (`break_it_down`, `make_goal_smart`, `create_activity_metric`, `first_session`, `schedule_program`, `see_progress`); the widget derives its `N/6` totals and numbering from the step list rather than a hardcoded count. The compact lower-left widget separates per-fractal local collapse from persisted dismissal; its single-line success-coloured progress action sits at the top-right and, like the parent `Go` action, expands the same fixed-width shell while preserving its typography hierarchy. The expanded view uses a bare parent step number aligned with the lettered sub-step codes, leaves position context to the footer navigation, and maintains a stable right-side status column for completion checks. Actionable sub-step rows are full-width keyboard-accessible deep links to the relevant feature; informational guidance remains static, and completed state relies on the shared check badge without redundant row gradients.
