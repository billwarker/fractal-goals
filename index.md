...
- Create Session is program-aware on the user’s timezone date. The shared Programs query establishes the currently underway program and block independently of whether a day is scheduled today; the header and per-user/root/program goal-scope preference therefore remain available for an unscheduled day or a manually chosen template. `GET /api/<root_id>/programs/active-days?date=YYYY-MM-DD` remains the date-keyed source for scheduled-day identity, template colors and requirement order, completion counts, and completed template ids. The page groups scheduled days by program id (so duplicate names never merge), advertises today’s day, prioritizes its templates, consistently styles program, block, and template names with their configured colors, auto-selects a single required session, and accepts calendar deep links via `program_day_id`. Program scope uses the program’s canonical selected goals and full tree lineage; manual goals outside that advisory scope remain selected until explicitly removed. Day-backed previews use authoritative server program scope, while dayless previews keep the established template-preview contract and intersect locked activity goals with the active program lineage client-side. Creation snapshots validated dayless active-program context for Session Detail, remains permissive for manual goals, and records `off_program_goal_ids` in `session_data.program_context` when scope is enabled. Calendar Day offers “Start session” only for today’s occurrences.
