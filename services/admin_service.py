...
from services.account_flags import FORCE_PASSWORD_CHANGE_PREFERENCE, revoke_user_sessions
from services._admin_common import (  # noqa: F401 - re-exported for existing imports
    as_aware_utc,
    generate_password,
    generate_secret,
    hash_invite_key,
)
from services._admin_beta_signups import _AdminBetaSignupsMixin


class AdminService(_AdminBetaSignupsMixin):
    ...
    def update_user(
        ...
        if "is_active" in data and not data["is_active"]:
            # Reactivation must not revive tokens issued before the suspension.
