...
function ResizeHandle({ edge, onMouseDown, visible = false }) {
    ...
    const style = {
        position: 'absolute',
        zIndex: 20,
        background: visible ? 'color-mix(in srgb, var(--color-brand-primary, var(--color-primary)) 20%, transparent)' : 'transparent',
        borderRadius: visible ? 4 : undefined,
    };
    if (isLeft) {
        style.left = 0;
    ...
    return <div aria-hidden="true" data-resize-edge={edge} style={style} onMouseDown={(event) => onMouseDown(event, edge)} />;
}
...
function ProfileWindowLayout({
    ...
    const onPointerCellChangeRef = useRef(onPointerCellChange);
    ...
    useEffect(() => {
        onPointerCellChangeRef.current = onPointerCellChange;
    }, [onPointerCellChange]);
    ...
    useEffect(() => {
        if (!rescaleOnBoundsChange) return;
        if (!bounds) return;
    ...
    const startResize = useCallback((event, windowId, edge) => {
        if (!windowsEditable && selectedWindowId !== windowId) return;
    ...
    }, [bounds, grid.panels, selectedWindowId, windowsEditable]);
    ...
    const getPointerCell = useCallback((event) => {
        const rect = containerRef.current?.getBoundingClientRect();
        if (!rect) return null;
        const activeBounds = bounds || {
            columns: Math.max(MIN_PANEL_W, Math.floor(rect.width / GRID_UNIT)),
            rows: Math.max(MIN_PANEL_H, Math.floor(rect.height / GRID_UNIT)),
        };
        const columns = Math.max(MIN_PANEL_W, activeBounds.columns || GRID_COLUMNS);
        const rows = Math.max(MIN_PANEL_H, activeBounds.rows || GRID_ROWS);
        const rawX = Math.floor((event.clientX - rect.left) / GRID_UNIT);
        const rawY = Math.floor((event.clientY - rect.top) / GRID_UNIT);
        const x = clamp(rawX, 0, columns - 1);
        ...
        return {
            ...
            columns,
            rows,
            fromRight: columns - x - 1,
            fromBottom: rows - y - 1,
            relativeX: columns > 1 ? x / (columns - 1) : 0,
    ...
    const handlePointerMove = useCallback((event) => {
        onPointerCellChangeRef.current?.(getPointerCell(event));
    }, [getPointerCell]);

    const handlePointerLeave = useCallback(() => {
        onPointerCellChangeRef.current?.(null);
    }, []);

    const handleBlankSpaceMouseDown = useCallback((event) => {
        if (event.target !== event.currentTarget) return;
        onBlankSpaceMouseDownRef.current?.(getPointerCell(event), event);
    }, [getPointerCell]);

    return (
        <div
            ref={containerRef}
            data-testid="grid-layout-root"
        ...
        >
            ...
            >
                {grid.panels.map((panel) => {
                    ...
                    const showResizeHandles = windowsEditable || isSelected;
                    ...
                    return (
                        ...
                        >
                            ...
                            {showResizeHandles && (
                                <>
                                    <ResizeHandle edge="top" visible={windowsEditable} onMouseDown={(event, edge) => startResize(event, boundedPanel.id, edge)} />
                                    <ResizeHandle edge="right" visible={windowsEditable} onMouseDown={(event, edge) => startResize(event, boundedPanel.id, edge)} />
                                    <ResizeHandle edge="bottom" visible={windowsEditable} onMouseDown={(event, edge) => startResize(event, boundedPanel.id, edge)} />
                                    <ResizeHandle edge="left" visible={windowsEditable} onMouseDown={(event, edge) => startResize(event, boundedPanel.id, edge)} />
                                    <ResizeHandle edge="top-left" visible={windowsEditable} onMouseDown={(event, edge) => startResize(event, boundedPanel.id, edge)} />
                                    <ResizeHandle edge="top-right" visible={windowsEditable} onMouseDown={(event, edge) => startResize(event, boundedPanel.id, edge)} />
                                    <ResizeHandle edge="bottom-left" visible={windowsEditable} onMouseDown={(event, edge) => startResize(event, boundedPanel.id, edge)} />
                                    <ResizeHandle edge="bottom-right" visible={windowsEditable} onMouseDown={(event, edge) => startResize(event, boundedPanel.id, edge)} />
                                </>
                            )}
                        </div>
                    );
