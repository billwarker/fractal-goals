...
- **Delivered as telemetry v2 on 2026-07-08:** `services/admin_usage_service.py` now accepts `start`/`end` date windows with legacy `days` fallback and a 365-day clamp. Payload includes `window`, DAU/WAU/MAU, signups, per-user page views/session creates/goal creates/total events, top product events/pages, full domain-event breakdown from `event_logs`, email health, event-table storage stats, retention settings, and BigQuery export state.
- **Admin API:** `GET /api/admin/usage`, `PATCH /api/admin/usage/retention`, and `POST /api/admin/usage/prune`, all behind the existing `_admin_service_or_response` admin gate.
- **Frontend:** `UsagePanel.jsx` stays merged into the Admin overview tab, now using the shared `DateRangeFilter`, a Chart.js DAU bar chart with axes, `EventsBreakdownTable`, and `UsageStoragePanel`.
- **Tests:** targeted coverage now lives in `tests/integration/test_admin_usage_api.py`, admin component Vitest files, shared date-range tests, and route visibility tests.
...
| 5. Admin usage dashboard | ✅ Upgraded on 2026-07-08 to usage telemetry v2: `GET /api/admin/usage?start=&end=` with legacy `days` fallback (365-day clamp), real DAU bar chart with axes, shared 7D/30D/90D/6M/1Y/custom date filter, full domain-event breakdown from `event_logs`, per-user total event counts, storage/retention/export-state panel, `PATCH /api/admin/usage/retention`, admin-only Logs nav/route/clear, `event_logs` quota decoupling, and BigQuery export service/job/docs/tests. Targeted verification: 44 backend tests passed (`--no-cov` slice) + 35 frontend tests passed. |
